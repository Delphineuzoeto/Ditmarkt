...
// toggle sud-menu options
document.addEventListener("DOMContentLoaded", () => {
  let menuSelections = document.querySelectorAll(".category__menu-section");

  menuSelections.forEach((menuSelection) => {
    let arrowIcon = menuSelection.querySelector(".arrow-icon");
    let subMenu = menuSelection.querySelector(".sub-menu");

    menuSelection.addEventListener("click", () => {
      // console.log("clicked");
      menuSelections.forEach((m) => {
        if (m !== menuSelection) {
          let mArrowIcon = m.querySelector(".arrow-icon");
          let mSubMenu = m.querySelector(".sub-menu");

          mArrowIcon.classList.remove("open");
          mSubMenu.classList.remove("rotate");
        }
      });
